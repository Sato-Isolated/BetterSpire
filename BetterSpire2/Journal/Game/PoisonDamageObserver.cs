...
internal static class PoisonDamageObserver
{
    ...
    internal static Task<IEnumerable<DamageResult>> Damage(PlayerChoiceContext choiceContext,
        ...
        CardModel? cardSource, PoisonPower poison)
    {
        Source? source = null;
        try
        {
            if (dealer == null && cardSource == null && target.Side == CombatSide.Enemy &&
        ...
        if (source == null) return CreatureCmd.Damage(choiceContext, target, amount, props, dealer, cardSource);
